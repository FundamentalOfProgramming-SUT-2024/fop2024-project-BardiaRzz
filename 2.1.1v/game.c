...
typedef struct
{
    char username[50];
    char password[50];
    long firstGameTime; // وقتی که اولین بازی را شروع کرده یا تمام کرده - epoch time
    int totalScore;     // مجموع همه امتیازات
    int totalGold;      // مجموع طلای کاربر
    int gamesPlayed;    // تعداد بازی‌های به پایان رسیده
} User;
